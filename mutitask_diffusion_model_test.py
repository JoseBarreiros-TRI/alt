...
def get_letter_vertices(letter: str, size: float):
    s = size
    h = s * 2
    w = s
...
def sample_shape_points(shape: str, size: float, n_samples: int, near_vertex_edges=False):
    if shape == 'ellipse':
        ...
        if near_vertex_edges:
            ...
            data = sample_along_edges_uniformly(vertices, n_samples)
    elif shape in list("ABCDEFGHIJK"):
        vertices = get_letter_vertices(shape.upper(), size)
        if near_vertex_edges:
            data = sample_near_vertex_edges(vertices, n_samples, max_spread=0.1)
        else:
            data = sample_along_edges_uniformly(vertices, n_samples)
...
def train_model(model, points, num_epochs, learning_rate, label=None, patience=early_stopping_patience, batch_size=None):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i in range(0, points_shuffled.size(0), batch_size):
            ...
            sqrt_1mab = sqrt_one_minus_alpha_bars[t - 1].unsqueeze(1)
            x_t = sqrt_ab * x0 + sqrt_1mab * epsilon
            ...
            model_input = torch.cat([x_t, task_conditioning, t_norm], dim=1)
...
def sample_points_with_trajectory(model, num_samples, initial_noise):
    x_t = initial_noise[:, :-1].to(device)
    task_encoding = initial_noise[:, -1].unsqueeze(1).to(device)

    trajectories = [x_t.cpu()]
    # Precompute sqrt(alpha_bar) and sqrt(1 - alpha_bar)
    sqrt_alpha_bars = torch.sqrt(alpha_bars)
    sqrt_1m_alpha_bars = torch.sqrt(1 - alpha_bars)
    ...
    for t in range(T, 0, -1):
        ...
        # t_norm = (t_cur - 1) / (T - 1)
        t_norm = (t - 1) / (T - 1)
        t_norm_tensor = torch.full((num_samples, 1), t_norm, device=device)
        model_input = torch.cat([x_t, task_encoding, t_norm_tensor], dim=1)

        pred_epsilon = model(model_input)
        # alpha_bar_t = alpha_bars[t-1]
        sqrt_ab = sqrt_alpha_bars[t - 1]
        sqrt_1mab = sqrt_1m_alpha_bars[t - 1]
    ...
    trajectories = torch.stack(trajectories, dim=1).numpy()
...
def train_and_plot_all(shape_types):
    fig, axs = plt.subplots(2, 2, figsize=(12, 12))
    axs = axs.flatten()
    save_dir = f"test_data/{'_'.join(shape_types)}"

    if os.path.exists(save_dir):
        print(f"Removing old folder: '{save_dir}'")
    ...
    for idx, (points_tensor_list, label, hidden_size, hidden_layers, ax, batch_size, data_type) in enumerate(configs):
        ...
        for shape_type in shape_types:
            ...
            cur_time_str += f"layer_size_{hidden_size_simple}_{hidden_layers_simple}_{hidden_size_complex}_{hidden_layers_complex}_{num_epochs}epochs_data_size_{low_data_samples}_{high_data_samples}"
            save_name_single = f"{shape_type}_multitask_model_is_{model_type}_Data_is_{data_type.capitalize()}_{cur_time_str}.png"
    ...
    save_name_loss = f"multitask_loss_curves_{cur_time_str}.png"
